  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        detalheMsg = errJson.error?.reason || errJson.error?.type || texto;
      } catch(e) {}
      return { statusCode: 200, headers, body: JSON.stringify({
        encontrado: false,
        mensagem: `DataJud retornou erro ${response.status}: ${detalheMsg}. Verifique se o processo existe e não é sigiloso.`
      }) };
    }
